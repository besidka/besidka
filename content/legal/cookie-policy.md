...
so this policy also lists what we keep in **localStorage**. Where the type matters
...
Two things follow, and we hold ourselves to both:
...
- **Nothing is pre-ticked.** Optional categories start off. We do not treat
...
| --- | --- | --- | --- | --- |
| `cookies_consent` | Cookie | Besidka | Remembers the cookie choice you made, so we do not ask again and can honour it | 180 days |
...
| `nuxt-color-mode` | Cookie | Besidka | A cookie of the same name for the same purpose. In the current setup the theme is kept in localStorage and this cookie is not written; we list it because it is part of the theme setting, and we clear it if you deny preferences | Not set in the current setup |
...
We have **no analytics cookies and no marketing cookies**, so those categories
...
### How we count visits instead

We count page views and a small number of interface events (for example that a
...
We do not embed third-party advertising or analytics scripts, so no other
company sets a cookie through our pages.

Two things happen off our site and are worth knowing about:
...
When you make a choice in the banner we keep a record of it, so we can show the
...
- If you turn the **preferences** category off, we delete the localStorage keys
...
- The `better-auth.last_used_login_method` cookie is set by the sign-in library
  rather than by us, and it is deleted too when you deny the preferences
...
If we add or remove a cookie or a storage key, we update this table and the date
at the top. If a change means we need new consent, we will ask you again before
