...
- **BYOK — Bring Your Own API Key** — Use your API keys from any supported LLM provider (Anthropic, Google, OpenAI, xAI, DeepSeek, Moonshot AI, Qwen), route through gateways (Vercel, Cloudflare, OpenRouter), and search with Brave or Exa. Only pay for what you use. No subscriptions, no markup
- **Multiple AI Models** — Switch between the latest models from all providers in one place
- **Web Search** — Ground AI answers with real-time web context; use native provider search, Brave, or Exa
- **Reasoning Mode** — Step-by-step thinking for complex questions
- **AI Image Generation** — Generate one image per request with supported OpenAI and Google models, then preview or download it from your private Generated by AI file library; generated images count toward your storage quota
