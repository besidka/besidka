...
async function getGetHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/exa/index.get'
  )
...
async function getPostHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/exa/index.post'
  )
...
async function getDeleteHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/exa/index.delete'
  )

  return module.default
}

describe('exa key API', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    vi.unstubAllGlobals()
  ...
  it('never returns the stored key to the client', async () => {
    ...
    await postHandler({ body: { apiKey: 'sk-exa-secret-key' } } as any)

    const response = await getHandler({} as any)

    expect(JSON.stringify(response)).not.toContain('sk-exa-secret-key')
    expect(response).toEqual({ hasKey: true })
  })

  it('looks the key up under the exa provider id on the GET route',
    async () => {
      const dbMock = createDbMock()
      ...
      expect(dbMock.spies.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ provider: 'exa' }),
        }),
      )
    })
