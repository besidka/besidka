...
async function getGetHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/brave/index.get'
  )
...
async function getPostHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/brave/index.post'
  )
...
async function getDeleteHandler() {
  const module = await import(
    '../../../server/api/v1/profiles/keys/brave/index.delete'
  )

  return module.default
}

describe('brave key API', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    vi.unstubAllGlobals()
  ...
  it('never returns the stored key to the client', async () => {
    ...
    await postHandler({ body: { apiKey: 'sk-brave-secret-key' } } as any)

    const response = await getHandler({} as any)

    expect(JSON.stringify(response)).not.toContain('sk-brave-secret-key')
    expect(response).toEqual({ hasKey: true })
  })

  it('looks the key up under the brave provider id on the GET route',
    async () => {
      const dbMock = createDbMock()
      ...
      expect(dbMock.spies.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ provider: 'brave' }),
        }),
      )
    })
