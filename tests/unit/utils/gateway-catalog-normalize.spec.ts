...
describe('fetchVercelGatewayCatalog', () => {
  ...
  it('reads toolCall from the tool-use tag, not supported_parameters, so a '
    + 'model reporting tools support only in supported_parameters does not '
    + 'earn it', async () => {
    mockFetchOnce({
      data: [
        {
          id: 'test-provider/parameters-only-model',
          name: 'Parameters Only Model',
          type: 'language',
          context_window: 32000,
          tags: ['vision'],
  ...
  it('reads toolCall as true from tool_choice alone, without tools',
    async () => {
      mockFetchOnce({
        data: [
          {
            id: 'test-author/tool-choice-only-model',
            name: 'Tool Choice Only Model',
            context_length: 8192,
...
describe('fetchCloudflareGatewayCatalog default-format enrichment', () => {
  ...
  it('never mistakes the default-format UUID for a model id', async () => {
    ...
    /**
     * `gptOss120bProperties.id` (the UUID) equals the marketplace model's
     * `id` here on purpose — if the join were wrongly keyed on `id === id`
     * instead of `marketplace.id === default.name`, this would incorrectly
     * match and `toolCall` would read `true` from `function_calling: 'true'`.
     * It must stay at its unenriched placeholder instead.
     */
  ...
  it('coerces the string "false" from function_calling into a real boolean',
    async () => {
      mockCloudflareCatalogFetch({
        marketplace: marketplaceCatalog,
        properties: {
