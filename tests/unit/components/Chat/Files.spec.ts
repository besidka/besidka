...
describe('Chat/Files', () => {
  ...
  it('excludes assistant image file parts, generated or not, from the '
    + 'attachments carousel — they render via ChatGeneratedImage instead',
  async () => {
    const wrapper = await mountSuspended(Files, {
      props: {
        message: {
    ...
    expect(wrapper.findAll('img')).toHaveLength(0)
    expect(wrapper.find('.carousel-item').exists()).toBe(false)
  })

  it('still renders a genuine user-uploaded image attachment', async () => {
    const wrapper = await mountSuspended(Files, {
      props: {
        message: {
          id: 'message-1',
