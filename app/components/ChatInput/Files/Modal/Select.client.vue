<template>
  <div class="flex flex-col h-full min-h-[300px]">
    ...
    <div
      class="join grid grid-cols-3 w-full shrink-0 mb-3"
      role="group"
      aria-label="Filter files by source"
    >
      <button
        type="button"
        class="join-item btn btn-sm min-h-11 min-w-0 px-2 sm:min-h-8"
        :class="source === 'all' ? 'btn-accent' : 'btn-ghost'"
      ...
      <button
        type="button"
        class="join-item btn btn-sm min-h-11 min-w-0 px-2 sm:min-h-8"
        :class="source === 'upload' ? 'btn-accent' : 'btn-ghost'"
      ...
      <button
        type="button"
        class="join-item btn btn-sm min-h-11 min-w-0 px-2 sm:min-h-8"
        :class="source === 'assistant' ? 'btn-accent' : 'btn-ghost'"
        :aria-pressed="source === 'assistant'"
        aria-label="Generated by AI"
        data-testid="files-source-assistant"
        @click="source = 'assistant'"
      >
        <Icon name="lucide:sparkles" size="12" aria-hidden="true" />
        <span class="sm:hidden">AI</span>
        <span class="max-sm:hidden">Generated by AI</span>
      </button>
    </div>
...
const emptyStateTitle = computed<string>(() => {
  ...
  if (source.value === 'assistant') {
    return 'No AI-generated files yet'
  }
...
const emptyStateDescription = computed<string>(() => {
  if (search.value) {
    return 'Try a different search term or file source'
  }

  if (source.value === 'assistant') {
    return 'Images created in chat will appear here'
  }

  if (source.value === 'upload') {
    return 'Upload files to add them to your library'
  }

  return 'Upload a file or create an image in chat to get started'
})
