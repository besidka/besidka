...
- Separate uploaded-file and generated-image landing statistics from existing
  provenance metadata without adding a second counter store.
...
table rebuild. The implementation does not apply migrations automatically.
A read-only audit now confirms the table is present in preview and absent in
production. Production rollout must still follow the guarded D1 checklist.
...
another request acquired after expiry. The additive migration is present in
preview and remains absent from production.
...
The three source choices use boxed DaisyUI tabs consistent with the existing
`Select existing files` and `Upload new files` switcher.
...
The attachment filename replaces any stale display-name suffix with the
extension derived from the authoritative D1 MIME type. This keeps converted
WebP uploads consistent across `Content-Type`, filename, and bytes even when
the retained display name originally ended in `.jpg` or `.png`.
...
Aggregate landing metrics are derived from canonical D1 records rather than a
second counter:

- `uploadedFiles` counts `files.source = 'upload'`;
- `generatedImages` counts `files.source = 'assistant'` with `type LIKE
  'image/%'`;
- the existing `files` value remains the total for API compatibility.

The Nitro/KV cache name and client query use a semantic image-generation
version rather than a generic ordinal. Active PR previews share cache
infrastructure, so generic names such as `landing-stats-v3` can collide with an
unrelated feature branch that has a different response schema.

The public stats route skips session enrichment. Its request wide event and the
scheduled cache-refresh wide event include only those counts and the cache
timestamp. The scheduled event is explicitly shipped to the configured Axiom
drain because it has no HTTP lifecycle to auto-emit it. Neither aggregate event
contains a prompt, filename, file ID, user ID, storage key, content, or provider
key. These values measure currently persisted records, so deletion and
retention can reduce them; a lifetime total would require a separate durable
analytics source.
...
- The home statistics render separate uploaded-file and generated-image cards.
- Stats query tests prove assistant non-image files do not increase the image
  total, and scheduled-job tests prove safe aggregates reach the Axiom drain.
...
1. Preserve the preview evidence and take a Time Travel bookmark before any
   further preview schema change.
2. Confirm the additive lock table and current row counts in `chat-preview`.
3. Keep the verified Google path and run the remaining opt-in OpenAI provider
   test in preview.
4. Verify storage counts, generated-file provenance, active sharing, lease
   recovery, landing aggregates, error rates, duration, and provider charges.
...
| Does this require a migration? | Yes, one additive lock table with no foreign keys. | Cross-request serialization needs atomic D1 state; preview has the table and production does not. |
