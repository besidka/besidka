...
- Generate an additive `image_generation_locks` migration with no foreign key,
  cascade, table rebuild, or destructive statement; leave it unapplied.
...
- Use the stored filename with safe `Content-Disposition` encoding.
...
- Audit the additive lock migration and keep it unapplied.
- Perform opt-in OpenAI and Google preview smoke tests when credentials exist.
...
- Full Vitest passes with 108 files and 906 tests.
- Migration-clean tests pass with 3 tests, and affected selection passes with
  78 tests.
- The affected-test mapping audit finds 98 mapped test files and 0 missing.
...
The orchestrator will create the following reviewable groups. Their hashes
remain pending until the commits exist.
