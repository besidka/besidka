 ...
 * toggle, and (indirectly, via `isGatewayToolAllowed`) the server-side send
 * gate, so the three can never drift apart again. `hasNativeSignal` comes
 * from whatever the gateway's raw catalog already reports (OpenRouter's
 * `web_search_options`, Vercel's `web-search` tag) — a genuine per-model
 * fact, kept as `'native'` regardless of the image-generation exclusion
 * below. Absent that, any model this gateway can universally search gets
 * `'universal'`, UNLESS it's a confirmed image-generation model: a globe on
 * an image model would recreate the exact "Image input badge on a
 * generation model" mislabeling this round exists to fix. An unknown
 * (`undefined`) image-generation status is not treated as an exclusion —
 * only positive evidence blocks the universal resolution.
 */
export function resolveGatewayWebSearchSupport(input: {
  ...
  return isGatewayToolAllowed(input.gatewayId, 'web_search')
    ? 'universal'
    : undefined
}
